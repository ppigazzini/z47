...
void nanCheck(char* s02) {
  int len = stringByteLength(s02);
  if (len > 2) {
    for (int ix = 2; s02[ix]!=0; ix++) {
      if (s02[ix] == 'n' && s02[ix-1] == 'a' && s02[ix-2] == 'n') {
        if (s02[0] == '(' && s02[ix+1] != 0) {
          strcpy(s02, "(NaN");
        } else if (s02[0] == ';' && s02[len-1] == ')' && s02[ix+1] != 0 && s02[ix+2] != 0) {
          strcpy(s02, ";NaN)");
        }
      }
    }
...
void eformat(char* s02, const char* s01, double inreal, uint8_t prec, const char* s05) {
  ...
  if ((fabs(inreal) > 1000000.0 || fabs(inreal) < 0.001) && inreal != 0.0) {
    snprintf(s03, sizeof(s03), "%.*e", prec, inreal);
  }
  else {
    snprintf(s03, sizeof(s03), "%.*f", prec, inreal);
  }
  strcpy(s02, s01);
...
//*****************************************************************************************************************************************************************************
//** The below piece was generated by free ChatGpt AI as a quick replacement of the eng() function previously used, which used a handful of functions not fully supported by nano
//** This does the same without the math functiones required

/* GOOD OPTION, not working on hardware though due to nano
...
static char *eng(char *result, double value, int digits) {
    char *sign = (value < 0.0) ? "-" : "";
    int expof10 = 0;
    double mantissa = fabs(value);

    if (mantissa == 0.0) {
        sprintf(result, "%s%.*f", sign, digits, 0.0);
        return result;
    }
    ...
    int significant_digits = digits;
    if (expof10 >= digits) {
        significant_digits = digits;
        sprintf(result, "%s%.*fE%+d", sign, significant_digits - 1, mantissa, expof10);
    } else if (expof10 >= -2 && expof10 < digits) {
        sprintf(result, "%s%.*f", sign, digits - expof10, mantissa * pow1(10, expof10));
    } else {
        significant_digits = digits;
        sprintf(result, "%s%.*fE%+d", sign, significant_digits - 1, mantissa, expof10);
    }
...
static char* eng(char* result, double value, int digits) {
    ...
    if ((sig_fix > sig_sci) || (sig_fix == sig_sci && value >= 1e-3 && value < 1e+4)) {
    ...
    // Uncomment these for debugging:
     //printf("[DEBUG] FIX='%s' SIG=%d\n", buf_fix, sig_fix);
     //printf("[DEBUG] SCI='%s' SIG=%d\n", buf_sci, sig_sci);
