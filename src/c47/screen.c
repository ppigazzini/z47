...
static bool_t displayTrueFalse(calcRegister_t regist) {
  ...
  static void _refreshRegisterLine(calcRegister_t regist, bool_t restoreRegisterT) {
    ...
    if((calcMode != CM_BUG_ON_SCREEN) && !GRAPHMODE && (calcMode != CM_LISTXY)) {               //JM
      ...
      else if((!checkHP && restoreRegisterT == RESTORE_T && regist == REGISTER_T && regist < REGISTER_X + min(displayStack, origDisplayStack)) || regist < REGISTER_X + min(displayStack, origDisplayStack) || (lastErrorCode != 0 && regist == errorMessageRegisterLine) || (temporaryInformation == TI_VIEW_REGISTER && regist == REGISTER_T)) {
        prefixWidth = 0;
