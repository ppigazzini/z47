 ...
 ***********************************************/
  ...
  void twoPowError(void) {
    ...
    sprintf(errorMessage, "cannot calculate 2" STD_SUP_BOLD_x " for %s", getRegisterDataTypeName(REGISTER_X, true, false));
...
void twoPowReal(void) {
  if(real34IsInfinite(REGISTER_REAL34_DATA(REGISTER_X)) && !getSystemFlag(FLAG_SPCRES)) {
    ...
    #if (EXTRA_INFO_ON_CALC_ERROR == 1)
      moreInfoOnError("In function twoPowReal:", "cannot use " STD_PLUS_MINUS STD_INFINITY " as X input of 2" STD_SUP_BOLD_x " when flag D is not set", NULL, NULL);
    #endif // (EXTRA_INFO_ON_CALC_ERROR == 1)
    return;
  }
