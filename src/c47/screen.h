...
char       letteredRegisterName(calcRegister_t regist);
  ...
  #define  halfSec_disp true
    ...
    #define clearScreenExcludingStatusBar(cnt)      do { lcd_fill_rect(0, 20, SCREEN_WIDTH, 240, LCD_SET_VALUE); clear_ul(); lastProgramRunStop = PGM_UNDEFINED; printf("CLEARFULLSCREEN EXCL SB Macro %u\n",(uint16_t)cnt);} while(0)
  #else //!MONITOR_CLRSCR
    ...
    #define clearScreenExcludingStatusBar(cnt)      do { lcd_fill_rect(0, 20, SCREEN_WIDTH, 240, LCD_SET_VALUE); clear_ul(); lastProgramRunStop = PGM_UNDEFINED;} while(0)
