...
bool_t itemNotAvail(int16_t itemNr) {
  ...
  void reallyRunFunction(int16_t func, uint16_t param) {
    ...
    if((indexOfItems[func].status & US_STATUS) == US_ENABLED || (indexOfItems[func].status & US_STATUS) == US_ENABL_XEQ) {
      if((programRunStop != PGM_RUNNING || getSystemFlag(FLAG_IGN1ER))  && calcMode != CM_NO_UNDO && !getSystemFlag(FLAG_SOLVING) && !getSystemFlag(FLAG_INTING) && !(currentSolverStatus & SOLVER_STATUS_USES_FORMULA)) {
...
/* 1907 */  { itemToBeCoded,                NOPARAM,                     "1907",                                        "1907",                                        (0 << TAM_MAX_BITS) |     0, CAT_FREE | SLS_ENABLED   | US_UNCHANGED | EIM_DISABLED | PTP_DISABLED     },
...
/* 1911 */  { itemToBeCoded,                NOPARAM,                     "1911",                                        "1911",                                        (0 << TAM_MAX_BITS) |     0, CAT_FREE | SLS_ENABLED   | US_UNCHANGED | EIM_DISABLED | PTP_DISABLED     },
...
/* 2236 */  { itemToBeCoded,                NOPARAM,                     "PLSTAT",                                      "PLSTAT",                                      (0 << TAM_MAX_BITS) |     0, CAT_NONE | SLS_UNCHANGED | US_UNCHANGED | EIM_DISABLED | PTP_DISABLED     },
