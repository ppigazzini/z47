...
const INVALID_MENU = 2933; // items.h: LAST_ITEM
...
// PGMSLV} {SIGMAnINF ADV_SLVQ ADV_SLVC ADV_SLVP NULL NULL}. Sum leads product on
...
const menu_ADV linksection(code_section) = [_]i16{ 1672, 1671, 2734, -3118, 1608, 2883, 1755, 1754, 2732, 1546, 1547, 2882, 2755, 1604, 1555, (if (option_slvp_poly) @as(i16, 1965) else 0), 0, 2884 };
...
const menu_ConvAng linksection(code_section) = [_]i16{ 2096, 2097, 2098, 2099, 2100, 2101, 2088, 2089, 2094, 2095, 2745, 2746, 0, 0, 0, 0, 0, 0 };
...
const menu_MATX linksection(code_section) = [_]i16{ 1536, 1704, 1529, 1530, 1602, -3141, 2726, 1526, 1739, 2737, 2478, 2739, 2497, 2498, 2729, 2728, 0, 2730, 1705, (if (option_eigen) @as(i16, 2727) else 0), 1578, 2710, 1702, -3141, 2704, 1628, 1704, 1745, 1449, 1436, 0, 0, (if (option_eigen) @as(i16, 1457) else 0), (if (option_eigen) @as(i16, 1456) else 0), 1535, (if (option_eigen) @as(i16, 1646) else 0), 1490, 1491, 1613, 1563, 1494, 1493, 2714, 2713, 1539, 2712, 2715, 1486, 1538, 1531, 1612, 1562, 2250, 2249 };
...
pub export fn savedspace(itemNr: i16) callconv(.c) bool_t {
    ...
    // !OPTION_SLVP_POLY: strike out ITM_SLVP on the packages that drop it.
