...
// glyph-row table from the three fonts) and fontBrowser (the on-screen font
...
pub export fn initFontBrowser() callconv(.c) void {
    ...
    while (g < standardFont.numberOfGlyphs) {
        glyphRow[@as(usize, numLinesNumericFont) + numLinesStandardFont] = glyphsOf(&standardFont)[g].charCode & 0xfff0;
        while (g < standardFont.numberOfGlyphs and ((glyphsOf(&standardFont)[g].charCode & 0xfff0) == glyphRow[@as(usize, numLinesNumericFont) + numLinesStandardFont])) {
            g += 1;
        }
    ...
    while (g < tinyFont.numberOfGlyphs) {
        glyphRow[@as(usize, numLinesNumericFont) + numLinesStandardFont + numLinesTinyFont] = glyphsOf(&tinyFont)[g].charCode & 0xfff0;
        while (g < tinyFont.numberOfGlyphs and ((glyphsOf(&tinyFont)[g].charCode & 0xfff0) == glyphRow[@as(usize, numLinesNumericFont) + numLinesStandardFont + numLinesTinyFont])) {
            g += 1;
        }
        numLinesTinyFont += 1;
    }
    ...
    if (comptime !dmcp_build) { // PC_BUILD
        if (@as(u16, numLinesNumericFont) + numLinesStandardFont + numLinesTinyFont > NUMBER_OF_GLYPH_ROWS) {
            _ = printf("In file defines.h NUMBER_OF_GLYPH_ROWS must be increased from %d to %d\n", @as(c_int, NUMBER_OF_GLYPH_ROWS), @as(c_int, @as(u16, numLinesNumericFont) + numLinesStandardFont + numLinesTinyFont));
...
pub export fn fontBrowser(unusedButMandatoryParameter: u16) callconv(.c) void {
    ...
    if (currentFntScr >= 1 and currentFntScr <= numScreensNumericFont) { // Numeric font
        ...
        while (y < minU16(@as(u16, currentFntScr) * NUMBER_OF_NUMERIC_FONT_LINES_PER_SCREEN, numLinesNumericFont)) : (y += 1) {
            _ = sprintf(tmpString, "%04X", @as(c_uint, if (glyphRow[y] < 0x8000) glyphRow[y] else glyphRow[y] -% 0x8000));
        ...
        _ = sprintf(tmpString, "%d/%d", @as(c_int, currentFntScr), @as(c_int, @as(u16, numScreensNumericFont) + numScreensStandardFont + numScreensTinyFont));
    ...
    } else if (currentFntScr > numScreensNumericFont and currentFntScr <= @as(u16, numScreensNumericFont) + numScreensStandardFont) { // Standard font
        x = 0;
        while (x <= 9) : (x += 1) {
            _ = showGlyphCode('0' + x, &standardFont, 50 + 20 * x, 20, vmNormal, false, false, false);
        }
        ...
        first = @as(u16, numLinesNumericFont) + (@as(u16, currentFntScr) - numScreensNumericFont - 1) * NUMBER_OF_STANDARD_FONT_LINES_PER_SCREEN;
        y = first;
        while (y < minU16(@as(u16, numLinesNumericFont) + (@as(u16, currentFntScr) - numScreensNumericFont) * NUMBER_OF_STANDARD_FONT_LINES_PER_SCREEN, @as(u16, numLinesNumericFont) + numLinesStandardFont)) : (y += 1) {
            _ = sprintf(tmpString, "%04X", @as(c_uint, if (glyphRow[y] < 0x8000) glyphRow[y] else glyphRow[y] -% 0x8000));
        ...
        _ = sprintf(tmpString, "%d/%d", @as(c_int, currentFntScr), @as(c_int, @as(u16, numScreensNumericFont) + numScreensStandardFont + numScreensTinyFont));
        _ = showString(tmpString, &standardFont, @as(u32, @intCast(SCREEN_WIDTH - stringWidth(tmpString, &standardFont, false, true))), 220, vmNormal, false, true);
    } else if (currentFntScr > @as(u16, numScreensNumericFont) + numScreensStandardFont and currentFntScr <= @as(u16, numScreensNumericFont) + numScreensStandardFont + numScreensTinyFont) { // Tiny font
        x = 0;
        while (x <= 9) : (x += 1) {
            _ = showGlyphCode('0' + x, &standardFont, 50 + 20 * x, 20, vmNormal, false, false, false);
        }
        ...
        first = @as(u16, numLinesNumericFont) + numLinesStandardFont + (@as(u16, currentFntScr) - numScreensNumericFont - numScreensStandardFont - 1) * NUMBER_OF_STANDARD_FONT_LINES_PER_SCREEN;
        y = first;
        while (y < minU16(@as(u16, numLinesNumericFont) + numLinesStandardFont + (@as(u16, currentFntScr) - numScreensNumericFont - numScreensStandardFont) * NUMBER_OF_STANDARD_FONT_LINES_PER_SCREEN, @as(u16, numLinesNumericFont) + numLinesStandardFont + numLinesTinyFont)) : (y += 1) {
            _ = sprintf(tmpString, "%04X", @as(c_uint, if (glyphRow[y] < 0x8000) glyphRow[y] else glyphRow[y] -% 0x8000));
            _ = showString(tmpString, &standardFont, 5, STANDARD_FONT_HEIGHT * (y - first) + 40, vmNormal, false, false);
            x = 0;
            while (x <= 15) : (x += 1) {
