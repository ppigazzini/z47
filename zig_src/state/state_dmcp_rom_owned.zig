...
// firmware. They were previously wrapped in C in the keyboard_state_legacy.c
// bridge (where the macros were in scope via the keyboard.c include chain); this
// module reconstructs them as lane-aware ROM trampolines so the bridge can be
// retired. On host (no power button / DMCP timers) they are trivial no-ops.
//
