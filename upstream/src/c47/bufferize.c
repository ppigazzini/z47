...
TO_QSPI const fInMim_t MimFunctionsType2[] =
  ...
  static bool_t isFunctionInMim(int16_t com, uint8_t fType) {
    const fInMim_t *table;
    uint_fast16_t   count;
    switch(fType) {
      case 0:  { table = MimFunctionsType0;     count = nbrOfElements(MimFunctionsType0);     break; }
      case 1:  { table = MimFunctionsType1;     count = nbrOfElements(MimFunctionsType1);     break; }
      case 2:  { table = MimFunctionsType2;     count = nbrOfElements(MimFunctionsType2);     break; }
      default: { table = MimFunctionsType3Conv; count = nbrOfElements(MimFunctionsType3Conv); break; }
    }
    for(uint_fast16_t i = 0; i < count; i++) {
      if(com == table[i].itemNr) {
        return true;
      }
    }
    return false;
      ...
      //Probably wrong place for this function?! Should Arrow be processed in buffercize.c in this case? //Switch statement better.
        ...
        switch(item) {
          ...
          default: {
            ...
            else if(isFunctionInMim(item, 2) || isFunctionInMim(item, 3)) {
