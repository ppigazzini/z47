...
static void setupRegCopyForwardCase(void) {
  seedBasicStack();
  seedRegister(2, 0xe0);
  seedRegister(3, 0xe4);
  seedRegister(5, 0xf0);
  seedRegister(6, 0xf4);
  stackParitySetRegCopyParams(ERROR_NONE, false, 5, 2, 2);
}

static void setupRegCopyBackwardCase(void) {
  seedBasicStack();
  seedRegister(2, 0xe0);
  seedRegister(3, 0xe4);
  seedRegister(4, 0xe8);
  seedRegister(5, 0xec);
  stackParitySetRegCopyParams(ERROR_NONE, false, 2, 3, 3);
}

static void setupRegCopyLoadCase(void) {
  seedBasicStack();
  seedRegister(2, 0xf0);
  seedRegister(3, 0xf4);
  seedRegister(6, 0xe0);
  seedRegister(7, 0xe4);
  stackParitySetRegCopyParams(ERROR_NONE, true, 2, 2, 6);
}
...
static void setupAdjustResultRealMatrixCase(void) {
  seedBasicStack();
  stackParitySetAdjustResultNoDropOutcome(true);
  stackParitySeedRegister(REGISTER_X, dtReal34Matrix, amNone, (const uint8_t[REAL34_SIZE_IN_BYTES]){0xac}, REAL34_SIZE_IN_BLOCKS);
}

static void setupAdjustResultComplexMatrixCase(void) {
  seedBasicStack();
  stackParitySetAdjustResultNoDropOutcome(true);
  stackParitySeedRegister(REGISTER_X, dtComplex34Matrix, amNone, (const uint8_t[TO_BYTES(COMPLEX34_SIZE_IN_BLOCKS)]){0xbc}, COMPLEX34_SIZE_IN_BLOCKS);
}

static void setupAdjustResultDefaultNoOpCase(void) {
  uint8_t complex_payload[TO_BYTES(COMPLEX34_SIZE_IN_BLOCKS)] = {0};

  seedBasicStack();
  stackParitySetAdjustResultNoDropOutcome(true);
  stackParitySeedRegister(REGISTER_X, dtShortInteger, amNone, (const uint8_t[TO_BYTES(SHORT_INTEGER_SIZE_IN_BLOCKS)]){0xcc}, SHORT_INTEGER_SIZE_IN_BLOCKS);
  complex_payload[0] = 0xcd;
  stackParitySeedRegister(2, dtComplex34, amNone, complex_payload, COMPLEX34_SIZE_IN_BLOCKS);
}
...
static void setupToRealShortIntegerCase(void) {
  seedBasicStack();
  lastIntegerBase = 16;
  stackParitySeedRegister(REGISTER_X, dtShortInteger, amNone, (const uint8_t[TO_BYTES(SHORT_INTEGER_SIZE_IN_BLOCKS)]){0xd0}, SHORT_INTEGER_SIZE_IN_BLOCKS);
}

static void setupToRealTimeCase(void) {
  seedBasicStack();
