...
void z47_registers_legacy_allocateLocalRegisters(uint16_t numberOfRegistersToAllocate) {
...
static void setupAllocateNamedVariableGrowCase(void) {
  stackParitySeedNamedVariableName(0, find_named_variable_hit);
}

static void setupAllocateNamedVariableFirstMemoryFullCase(void) {
  stackParitySetMemoryBlockAvailable(false);
}

static void setupAllocateNamedVariableGrowMemoryFullCase(void) {
  setupAllocateNamedVariableGrowCase();
  stackParitySetMemoryBlockAvailable(false);
}
...
static int runU16VoidCase(const char *name, const char *case_name, u16_void_fn oracle_fn, u16_void_fn zig_fn, void (*setup)(void), uint16_t arg) {
  stack_parity_snapshot_t expected_snapshot;
  stack_parity_snapshot_t actual_snapshot;
  int failures = 0;
...
int main(void) {
  ...
  failures += runAllocateNamedVariableCase("allocateNamedVariable", "first-alloc-ram-full", oracle_allocateNamedVariable, allocateNamedVariable, setupAllocateNamedVariableFirstMemoryFullCase, find_or_allocate_named_variable_first_alloc, dtReal34, REAL34_SIZE_IN_BLOCKS);
  failures += runAllocateNamedVariableCase("allocateNamedVariable", "grow", oracle_allocateNamedVariable, allocateNamedVariable, setupAllocateNamedVariableGrowCase, find_named_variable_miss, dtReal34, REAL34_SIZE_IN_BLOCKS);
  failures += runAllocateNamedVariableCase("allocateNamedVariable", "grow-ram-full", oracle_allocateNamedVariable, allocateNamedVariable, setupAllocateNamedVariableGrowMemoryFullCase, find_named_variable_miss, dtReal34, REAL34_SIZE_IN_BLOCKS);
  ...
  failures += runGetPointerCase("getRegisterDataPointer", oracle_getRegisterDataPointer, getRegisterDataPointer, setupNoOpCase, FIRST_NAMED_VARIABLE);
  ...
  failures += runGetPointerCase("getRegisterDataPointer", oracle_getRegisterDataPointer, getRegisterDataPointer, setupNoOpCase, FIRST_LOCAL_REGISTER);
  failures += runGetPointerCase("getRegisterDataPointer", oracle_getRegisterDataPointer, getRegisterDataPointer, setupNoOpCase, LAST_LOCAL_REGISTER + 1);

  failures += runGetU32Case("getRegisterTag", oracle_getRegisterTag, getRegisterTag, setupGlobalCase, REGISTER_X);
  failures += runGetU32Case("getRegisterTag", oracle_getRegisterTag, getRegisterTag, setupNoOpCase, FIRST_NAMED_VARIABLE);
  ...
  failures += runGetU32Case("getRegisterTag", oracle_getRegisterTag, getRegisterTag, setupNoOpCase, FIRST_LOCAL_REGISTER);
  failures += runGetU32Case("getRegisterTag", oracle_getRegisterTag, getRegisterTag, setupNoOpCase, LAST_LOCAL_REGISTER + 1);
  ...
  failures += runSetMaxLengthCase("setRegisterMaxDataLengthInBlocks", oracle_setRegisterMaxDataLengthInBlocks, setRegisterMaxDataLengthInBlocks, setupReservedLongIntegerCase, FIRST_RESERVED_VARIABLE + 40, 8);
  ...
  failures += runCopyCase("copySourceRegisterToDestRegister", oracle_copySourceRegisterToDestRegister, copySourceRegisterToDestRegister, setupNoOpCase, FIRST_NAMED_VARIABLE, REGISTER_Y);
