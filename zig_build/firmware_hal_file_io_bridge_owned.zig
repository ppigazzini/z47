...
const file_io_bridge_seekremove_owned = @import("firmware_hal_file_io_bridge_seekremove_owned.zig");
...
pub fn ioFileSeek(position: u32) void {
    file_io_bridge_seekremove_owned.ioFileSeek(position);
}

pub fn ioFileClose(io_write_enabled: *c_int, io_read_enabled: *c_int) void {
    file_io_bridge_openclose_owned.ioFileClose(io_write_enabled, io_read_enabled);
}
...
pub fn ioFileRemove(path: c_int, error_number: ?*u32) c_int {
    return file_io_bridge_seekremove_owned.ioFileRemove(path, error_number);
}
