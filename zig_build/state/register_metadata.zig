...
pub const RuntimeObjects = struct {
    ...
    pub fn addToCommand(self: RuntimeObjects, cmd: *std.Build.Step.Run) void {
        ...
        cmd.addArg("zig_bridge/state/" ++ "registers_legacy.c");
...
fn addRuntimeObject(
    ...
    const module = b.createModule(.{
        .root_source_file = b.path("zig_src/state/register_metadata.zig"),
        .target = target,
        .optimize = optimize,
        .strip = options.strip,
...
pub fn addToModule(
    ...
    module.addCSourceFile(.{ .file = b.path("zig_bridge/state/" ++ "registers_legacy.c"), .flags = c_flags });
