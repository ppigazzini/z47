...
- `build.zig` is the canonical maintained entrypoint. The repo-root `build.zig`
  stays small and routes work into `build/`.
...
- avoid adding new top-level roots; prefer adding paths under the existing
  `build/`, `src/`, `bridge/`, `.github/`, or `docs/` trees.

## Repo-Owned Source Domains

The ported calculator core lives under `src/`, split into three zones. The
zone split is the architectural one; the per-domain build objects under
`build/` are a separate carving and do not have to agree with it.
...
```bash
git ls-files 'src/**/*.zig' 'src/*.zig' | wc -l          # owners in total
git ls-files 'src/core/**/*.zig' | cut -d/ -f3 | sort | uniq -c   # per core domain
git ls-files 'build/**/*.zig' 'build/*.zig' | wc -l      # build-side files
```
...
- `../.github/project/retained-bridge-review.tsv` and
  ...
  ledger and guard for the remaining `bridge` seams
...
- internal implementation helpers that exist only behind a paired export shim use
  `*_owned.zig`, for example `src/core/analysis/solve_owned.zig`
...
- Keep `build.zig` small. Push domain-specific logic down into `build/`.
- Add new z47-owned build logic under `build/` or `.github/`, new live
  runtime Zig under `src/`, and do not place new z47-owned files under
...
- Keep `../README.md`, `docs/`, and the live `zig build --help` surface
