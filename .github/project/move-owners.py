...
def load_map(mf):
    m = {}
    for line in pathlib.Path(mf).read_text().splitlines():
        line = line.rstrip("\n")
        ...
        if module_of(old) != module_of(new):
            sys.exit(
                f"REJECT cross-module move (Zig forbids outside-module @import): {old} -> {new}"
            )
...
def main():
    ...
    if verify:
        ...
        for f in tracked_zig():
            for imp in IMP.findall(pathlib.Path(f).read_text()):
                if not os.path.exists(resolve(f, imp)):
                    print(f"DANGLING: {f} -> {imp}")
                    bad = 1
    ...
    for f, imp, tgt in edges:
        fnew = mp.get(f, f)
        tnew = mp.get(tgt, tgt)
        if f not in mp and tgt not in mp:
            continue
        newimp = os.path.relpath(tnew, os.path.dirname(fnew))
        if newimp != imp:
            rewrites[fnew].append((imp, newimp))
    # report
    print(
        f"# {len(mp)} moves, {sum(len(v) for v in rewrites.values())} import rewrites across {len(rewrites)} files"
    )
    for old, new in sorted(mp.items()):
        print(f"  MOVE {old} -> {new}")
    for fn in sorted(rewrites):
        for a, b in rewrites[fn]:
            print(f"  IMPORT [{fn}]  {a} -> {b}")
    if not apply:
    ...
    for fn, pairs in rewrites.items():
        ...
        for a, b in pairs:
            s = s.replace(f'@import("{a}")', f'@import("{b}")')
        pathlib.Path(fn).write_text(s)
    # build-system + audit path literals (all enumerated surfaces)
    reff = [x for x in extra_reffiles() if os.path.exists(x)]
    for d in REFDIRS:
        reff += subprocess.check_output(["git", "ls-files", f"{d}/**/*.zig"], text=True).split()
    for fn in set(reff):
        s = pathlib.Path(fn).read_text()
        orig = s
        for old, new in mp.items():
