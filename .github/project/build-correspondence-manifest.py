...
every src owner, collect the symbols it defines or exports, EXCLUDING pure
...
C_DEF_STUB = re.compile(
    r"^[ \t]+[A-Za-z_][A-Za-z0-9_\*\s]+?\b([a-zA-Z_][A-Za-z0-9_]*)\s*\([^;]*\)\s*\{", re.M
)
C_KEYWORDS = {
    "return",
    "else",
...
def zig_owned_symbols(rel: str, text: str) -> dict[str, str]:
    ...
    aliases = {
        a: str((here / pathlib.PurePosixPath(p)).with_suffix(""))
...
def main() -> int:
    ...
    if args.check:
        print(
            f"correspondence coverage: {len(covered)}/{len(c_files)} C files owned by symbol join"
        )
        ...
        agg = [
            cf
            for cf, ow in file_owners.items()
    ...
    lines = [
        "# src/c47 C file\tsrc owner(s)\tkind\tvia",
    ...
    print(
        f"wrote {MANIFEST}: {len(covered)} symbol rows, "
