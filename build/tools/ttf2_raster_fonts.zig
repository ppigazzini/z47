...
fn buildXlsxCommand(allocator: std.mem.Allocator, fonts_path: []const u8) std.mem.Allocator.Error![:0]u8 {
    return switch (builtin.target.os.tag) {
        .linux => allocPrintZ(
            allocator,
            "if command -v xlsxio_xlsx2csv >/dev/null 2>&1; then LD_LIBRARY_PATH=\"$HOME/.local/lib${{LD_LIBRARY_PATH:+:$LD_LIBRARY_PATH}}\" xlsxio_xlsx2csv -b \"{s}/sortingOrder.xlsx\" >/dev/null; else python3 build/tools/xlsx_to_sorting_csv.py \"{s}/sortingOrder.xlsx\" \"{s}/sortingOrder.xlsx.sortingOrder.csv\"; fi",
            .{ fonts_path, fonts_path, fonts_path },
        ),
        .macos => allocPrintZ(
            allocator,
            "if command -v xlsxio_xlsx2csv >/dev/null 2>&1; then DYLD_LIBRARY_PATH=\"$HOME/.local/lib${{DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}}\" xlsxio_xlsx2csv -b \"{s}/sortingOrder.xlsx\" >/dev/null; else python3 build/tools/xlsx_to_sorting_csv.py \"{s}/sortingOrder.xlsx\" \"{s}/sortingOrder.xlsx.sortingOrder.csv\"; fi",
            .{ fonts_path, fonts_path, fonts_path },
        ),
        else => allocPrintZ(
            allocator,
            "python3 build/tools/xlsx_to_sorting_csv.py \"{s}/sortingOrder.xlsx\" \"{s}/sortingOrder.xlsx.sortingOrder.csv\"",
