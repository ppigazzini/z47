...
void oracle_clearRegister(calcRegister_t regist) {
	...
	z47_registers_clearRegister(regist);
}
...
void oracle_fnRegClr(uint16_t unusedButMandatoryParameter) {
	...
	if((lastErrorCode = z47_registers_get_reg_clr_range(&s, &n)) == ERROR_NONE) {
...
void oracle_fnRegSwap(uint16_t unusedButMandatoryParameter) {
	...
	if((lastErrorCode = z47_registers_get_reg_swap_range(&s, &n, &d)) == ERROR_NONE) {
		if((d < s + n) && (s < d + n)) {
...
void oracle_fnRegCopy(uint16_t unusedButMandatoryParameter) {
	...
	if((lastErrorCode = z47_registers_get_reg_copy_params(&f, &s, &n, &d)) == ERROR_NONE) {
		if(f) {
			z47_registers_fnRegCopy(unusedButMandatoryParameter);
		}
		else if(s > d) {
			for(uint16_t i = 0; i < n; ++i) {
...
void oracle_fnRegSort(uint16_t unusedButMandatoryParameter) {
	...
	if((lastErrorCode = z47_registers_get_reg_clr_range(&s, &n)) == ERROR_NONE) {
		...
		if(lastErrorCode == ERROR_NONE) {
			z47_registers_sort_reg(s, (uint16_t)(s + n - 1));
		}
	}
	else {
		z47_stack_runtime_report_register_command_error(lastErrorCode);
...
void oracle_fnToReal(uint16_t unusedButMandatoryParameter) {
	...
	if(z47_stack_runtime_try_fn_to_real_long_integer()) {
		return;
	}

	if(z47_stack_runtime_try_fn_to_real_short_integer()) {
		return;
	}

	if(z47_stack_runtime_try_fn_to_real_time()) {
		return;
	}
	...
	z47_registers_fnToReal(unusedButMandatoryParameter);
}
