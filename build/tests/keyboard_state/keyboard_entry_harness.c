...
int main(void) {
  ...
  // Scenario 11: COMPLEX multiplication via the keyboard. Y = 1 + 2i, X = 3 + 4i,
  // click x (index 26) -> (1*3-2*4) + (1*4+2*3)i = -5 + 10i. Exercises the
  // cross-term complex multiply through the entry path.
  fnReset(CONFIRMED);
  resetOtherConfigurationStuff(true);
  reallocateRegister(REGISTER_Y, dtComplex34, 0, amNone);
  int32ToReal34(1, REGISTER_REAL34_DATA(REGISTER_Y));
  ...
  btnClicked(NULL, (gpointer)"26"); // *
  {
    ...
    decQuadToString((decQuad *)REGISTER_REAL34_DATA(REGISTER_X), rb);
    decQuadToString((decQuad *)REGISTER_IMAG34_DATA(REGISTER_X), ib);
    if(strcmp(rb, "-5") != 0 || strcmp(ib, "10") != 0) {
      printf("FAIL: (1+2i)*(3+4i): got %s, %si, expected -5, 10i\n", rb, ib);
      return 1;
    }
  }
