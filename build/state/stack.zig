...
fn addRuntimeObject(
    ...
    const module = b.createModule(.{
        .root_source_file = b.path("src/core/state/stack.zig"),
        .target = target,
        .optimize = optimize,
        .strip = options.strip,
...
pub fn addParityExecutable(
    ...
    exe.root_module.addCSourceFile(.{ .file = b.path("build/tests/stack_state/stack_state_fake_runtime.c"), .flags = &.{"-DZ47_STACK_STATE_RUNTIME=1"} });
    exe.root_module.addCSourceFile(.{ .file = b.path("build/tests/stack_state/stack_state_parity.c"), .flags = &.{} });
    exe.root_module.addCSourceFile(.{ .file = b.path("build/tests/stack_state/stack_state_oracle.c"), .flags = &.{} });
