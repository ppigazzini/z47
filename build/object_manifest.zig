...
//! MODULE graph is `@import` over `src/` -- what a reader navigates, gated by
//! `check-module-graph.py`. The OBJECT graph is what the linker builds: its nodes
//! are the build roots declared in `build/`, and its edges are undefined
...
pub fn addWriter(
    ...
    return b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            .root_source_file = b.path("build/tools/write_object_manifest.zig"),
            .target = target,
            .optimize = optimize,
        }),
    });
