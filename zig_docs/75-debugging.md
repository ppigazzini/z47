...
| malformed `.sav` / `.d47` state file | `zig build state_load_fuzz` -- the corpus through the real `doLoad`, checked for crash/hang/safety-panic AND for the outcome `expectations.txt` states | `saveload_parity` and `saveload_golden`, which round-trip valid files only |
...
`vendor_sanitizer_exclusions` in `../zig_build/common.zig`:
...
`comptime` block in `../zig_build/common.zig` now fails the build if any
