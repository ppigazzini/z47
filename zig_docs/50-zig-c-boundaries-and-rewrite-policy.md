...
| **Drive malformed input through the REAL path, not a unit stub.** | `zig_build/tests/pgm_run/malformed/` -- 27 `.p47` files (truncated at 12 offsets, corrupt magic, garbage/negative/overflowing size fields, all-zero and all-`0xFF` bodies) through the actual program-load code. NOT under a sanitizer, despite the harness name -- see the sanitizer note in [75-debugging.md](75-debugging.md); a finding here is a crash, a hang or a Zig safety panic. | `zig build pgm_load_fuzz` |
| **Sanitize the retained C on a lane that actually runs.** | **NOT HELD.** `test_asan` sets `sanitize_c`, and `common_c_flags` cancels it with `-fno-sanitize=undefined`; the linked binary carries zero sanitizer symbols. `both_asan` only BUILDS the simulators anyway. Zig ships no ASan runtime at all, so the `*_asan` names have never described these lanes. See [75-debugging.md](75-debugging.md). | none today |
