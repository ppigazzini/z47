...
| `../zig_build/solver/solve_rewrites.zig` | manual Zig entrypoint plus retained C boundary routing | filters imported `solver/solve.c`, re-adds retained body through `zig_bridge/solver/solve_retained.c`, and routes `fnPgmSlv` through `zig_src/solver/solve.zig` |
